"""Texas Hold'em state implementation for full 52-card deck."""
...
logger = logging.getLogger("poker_ai.games.texas_holdem.state")
...
def new_game(
    ...
    """
    Create a new game of Texas Hold'em poker.
    ...
    if n_players < 2 or n_players > 8:
        raise ValueError(f"Number of players must be between 2 and 8, got {n_players}")
    ...
    if card_info_lut:
        ...
        state = TexasHoldemPokerState(players=players, load_card_lut=False, **kwargs)
        state.card_info_lut = card_info_lut
    else:
        # Load massive files.
        state = TexasHoldemPokerState(players=players, **kwargs)
    return state


class TexasHoldemPokerState:
    """The state of a Texas Hold'em Poker game at some given point in time.
    ...
    def __init__(
        self,
        players: List[TexasHoldemPokerPlayer],
        small_blind: int = 50,
        big_blind: int = 100,
        lut_path: str = ".",
        ...
        if n_players < 2 or n_players > 8:
            raise ValueError(
                f"Texas Hold'em requires 2-8 players but {n_players} "
                f"were provided."
            )
        ...
        # Full deck: ranks 2-14 (2 through Ace)
        self._table = PokerTable(
            players=players, 
            pot=players[0].pot, 
            include_ranks=list(range(2, 15))  # Full deck: 2-A
        )
    ...
    def __repr__(self):
        ...
        return f"<TexasHoldemPokerState player_i={self.player_i} betting_stage={self._betting_stage}>"

    def apply_action(self, action_str: Optional[str]) -> TexasHoldemPokerState:
    ...
    def load_card_lut(
        ...
        if pickle_dir:
            ...
            file_names = [
                "texas_holdem_preflop_lossless.pkl",
                "texas_holdem_flop_lossy_2.pkl",
                "texas_holdem_turn_lossy_2.pkl",
                "texas_holdem_river_lossy_2.pkl",
            ]
            ...
            # Try texas holdem specific file first
            texas_holdem_path = lut_path + "/texas_holdem_card_info_lut.joblib"
            if os.path.isfile(texas_holdem_path):
                card_info_lut = joblib.load(texas_holdem_path)
            elif os.path.isfile(lut_path + "/card_info_lut.joblib"):
    ...
    @property
    def private_hands(self) -> Dict[TexasHoldemPokerPlayer, List[Card]]:
    ...
    def info_set(self) -> str:
        ...
        try:
            ...
            if self.betting_stage not in {"terminal", "show_down"}:
                logger.warning("Cards not found in lut, using default")
            return "default info set, please ensure you load it correctly"
        # Convert history from a dict of lists to a list of dicts
        info_set_dict = {
