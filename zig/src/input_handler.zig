...
pub const InputHandler = struct {
    ...
    pub fn getRaiseAmountInput(self: *InputHandler, current_amount: u32, min_amount: u32, max_amount: u32) !InputEvent {
        if (try self.readChar()) |char| {
            switch (char) {
                '+', '=' => {
    ...
    pub fn getPlayerAction(
        ...
        if (!is_valid) {
            return InputEvent{ .player_action = .{ .action = .invalid, .amount = null } };
        }
        ...
        if (action == .raise) {
            ...
            return InputEvent{ .player_action = .{ .action = action, .amount = amount } };
        }

        return InputEvent{ .player_action = .{ .action = action, .amount = null } };
    }
    ...
    pub fn handleMenu(self: *InputHandler, menu_items: []const []const u8, selected_index: *usize, timeout_ms: ?u32) !InputEvent {
