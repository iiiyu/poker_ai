...
pub const GameDisplay = struct {
    ...
    fn drawPlayer(
        ...
        // Player status
        self.moveCursor(pos.y + 1, pos.x - 5);
        const status = if (!p.is_active) "FOLDED" 
                      else if (p.is_all_in) "ALL-IN"
                      else if (p.is_sitting_out) "SITTING OUT"
                      else "ACTIVE";
        
        if (self.use_color) {
            const color = if (!p.is_active) Colors.DIM 
                         else if (p.is_all_in) Colors.RED
                         else Colors.RESET;
    ...
    fn drawStatusBar(self: *GameDisplay, players: []const Player) !void {
        ...
        self.moveCursor(y, x);
        if (self.use_color) {
            std.debug.print("{s}Active Players: {d} | Total Chips: {d} | [Q]uit{s}", 
                .{ Colors.DIM, active_count, total_chips, Colors.RESET });
        } else {
            std.debug.print("Active Players: {d} | Total Chips: {d} | [Q]uit", 
                .{ active_count, total_chips });
        }
    }
    ...
    pub fn showHandResult(
        self: *GameDisplay,
        players: []const Player,
        winner_indices: []const u8,
        winning_hand_description: []const u8,
        pot_distribution: []const u32
    ) !void {
        ...
        for (players, 0..) |p, i| {
            ...
            if (p.hasValidCards()) {
                ...
                if (winnings > 0) {
                    if (self.use_color) {
                        std.debug.print("{s}Player {d}: {s} {s} - WON ${d}{s}", 
                            .{ Colors.BOLD, player_index + 1, card1_str, card2_str, winnings, Colors.RESET });
                    } else {
                        std.debug.print("Player {d}: {s} {s} - WON ${d}", 
                            .{ player_index + 1, card1_str, card2_str, winnings });
                    }
                } else {
                ...
                if (winnings > 0) {
                    if (self.use_color) {
                        std.debug.print("{s}Player {d}: (folded) - WON ${d}{s}", 
                            .{ Colors.BOLD, player_index + 1, winnings, Colors.RESET });
                    } else {
