...
pub const StrategyAggregator = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        ...
        for (shards) |*shard| {
            shard.* = .{
                .rwlock = std.Thread.RwLock{},
                .entries = std.HashMap(info_set.HashType, StrategyEntry, std.hash_map.AutoContext(info_set.HashType), std.hash_map.default_max_load_percentage).init(allocator),
                .allocator = allocator,
    ...
    pub fn updateStrategy(
        ...
        if (!exists) {
            ...
            if (!shard.entries.contains(hash)) {
                const new_entry = try StrategyEntry.init(shard.allocator, hash, @intCast(strategy.len));
    ...
    pub fn batchUpdate(
        ...
        var shard_updates = try self.allocator.alloc(std.ArrayList(StrategyUpdate), self.num_shards);
        defer {
        ...
        for (self.shards, 0..) |*shard, i| {
            ...
            for (shard_updates[i].items) |update| {
                if (shard.entries.getPtr(update.hash)) |entry| {
                    ...
                    // Create new entry
                    var new_entry = try StrategyEntry.init(shard.allocator, update.hash, @intCast(update.strategy.len));
        ...
        for (other.shards, 0..) |*other_shard, shard_idx| {
            ...
            while (iterator.next()) |other_entry| {
                if (self_shard.entries.getPtr(other_entry.key_ptr.*)) |self_entry| {
                    ...
                    // Copy entry
                    const new_entry = try StrategyEntry.init(self_shard.allocator, other_entry.key_ptr.*, other_entry.value_ptr.num_actions);
                    @memcpy(new_entry.strategy_sum, other_entry.value_ptr.strategy_sum[0..other_entry.value_ptr.num_actions]);
